...
export function useSchedule() {
  ...
  const getPostsByDate = (date: Date) => {
    return scheduledPosts.value.filter((post) => post.date.toDateString() === date.toDateString());
  };
  ...
  const currentMonthPosts = computed(() => {
    ...
    return scheduledPosts.value.filter(
      (post) => post.date.getFullYear() === year && post.date.getMonth() === month
    );
  });
