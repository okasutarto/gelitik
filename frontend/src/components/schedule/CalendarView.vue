...
const getPostsForDay = (date: Date) => {
  return currentMonthPosts.value.filter(post => 
...
<template>
  <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
    <!-- Weekday Headers -->
    <div class="grid grid-cols-7 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
      <div
    ...
    <!-- Calendar Grid -->
    <div class="grid grid-cols-7 auto-rows-fr bg-slate-200 dark:bg-slate-700 gap-px">
      <div
        v-for="(day, index) in calendarDays"
        :key="index"
        :class="[
          'min-h-[120px] p-2 bg-white dark:bg-slate-800 transition-colors',
          !day.isCurrentMonth && 'bg-slate-50/50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-600',
