...
const sortedVideos = computed(() => {
  ...
  return [...props.videos].sort((a, b) => {
    const metricA = (sortBy.value === 'engagement') 
      ? ((a.like_count || 0) + (a.comment_count || 0) + (a.share_count || 0)) / (a.view_count || 0.00001) * 100
      : a[sortBy.value === 'views' ? 'view_count' : sortBy.value];
    const metricB = (sortBy.value === 'engagement')
      ? ((b.like_count || 0) + (b.comment_count || 0) + (b.share_count || 0)) / (b.view_count || 0.00001) * 100
  ...
  return {
    ...
    datasets: [{
      ...
      data: videos.map(v => {
        ...
        return ((v.like_count || 0) + (v.comment_count || 0) + (v.share_count || 0)) / (v.view_count || 0.00001) * 100;
      }),
...
<template>
  <div class="neo-card border-neo-3 border-black neo-hover-lift h-full flex flex-col">
    <!-- Header -->
    ...
    <div class="mt-6 space-y-3">
      ...
      >
        <div class="text-lg font-bold w-12 text-slate-600 dark:text-slate-300 shrink-0">
          {{ index + 1 }}
        </div>
        <div class="flex-1 min-w-0 flex gap-3">
          <div class="flex-1">
            <img
              :src="video.cover_image_url || 'https://via.placeholder.com/64x64'"
