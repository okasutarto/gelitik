    ...
    if (props.platform === 'all') {
        ...
        if (hasHistoryData.value && props.followerHistory) {
            ...
            if (igKey && props.followerHistory[igKey].length > 0) {
                const igMap = new Map(
                    props.followerHistory[igKey].map((e) => [e.date, e.followers])
                )
                datasets.push({
                    label: 'Instagram',
            ...
            if (props.followerHistory['tiktok']?.length > 0) {
                const ttMap = new Map(
                    props.followerHistory['tiktok'].map((e) => [e.date, e.followers])
                )
                datasets.push({
                    label: 'TikTok',
...
<template>
    <div class="brutal-card brutal-hover-lift rounded-none p-6">
        <div
            v-if="platform === 'tiktok' && !historicalData?.followers?.length"
        ...
        <div v-else-if="platform === 'all' && !hasHistoryData" class="p-8 text-center">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">
                Follower snapshots are recorded each time you view your analytics. Check back after
                a few days.
            </p>
        </div>
        <div v-else>
