...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    >
      <!-- Header -->
      <div class="flex items-center justify-between p-5 border-b-4 border-black">
        <h3 class="text-xl font-black text-slate-900 dark:text-white">Create New Post</h3>
        <button
          @click="$emit('close')"
      ...
      <div class="p-6 space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-2">
            <label class="text-sm font-bold text-slate-900 dark:text-white">Time</label>
            <div class="relative">
              <input
                v-model="time"
                type="time"
                class="w-full h-12 pl-12 pr-4 bg-white dark:bg-slate-700 border-neo-3 border-black text-slate-900 dark:text-white font-semibold focus:border-neo-accent outline-none text-sm"
              />
              <Clock class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" :size="20" />
            </div>
          </div>
        </div>
        ...
        <!-- Content -->
        <div class="space-y-2">
          <label class="text-sm font-bold text-slate-900 dark:text-white">Caption</label>
          <div class="relative">
            <textarea
              v-model="content"
              rows="4"
        ...
        >
          ...
          <p class="text-sm font-bold text-slate-900 dark:text-white">Upload Media</p>
          <p class="text-xs font-medium text-slate-500 mt-2">Drag and drop or click to browse</p>
        </div>
      </div>

      <!-- Footer -->
      <div class="p-5 border-t-4 border-black flex justify-end gap-4 bg-slate-50 dark:bg-slate-700">
