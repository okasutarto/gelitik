...
const getPlatformBadge = (platform: Platform) => {
  const configs: Record<Platform, { bg: string; text: string; dot: string }> = {
    all: { bg: 'bg-primary-50 dark:bg-primary-900/30', text: 'text-primary-600 dark:text-primary-400', dot: 'bg-primary-500' },
    instagram: { bg: 'bg-pink-50 dark:bg-pink-900/30', text: 'text-pink-600 dark:text-pink-400', dot: 'bg-pink-500' },
    tiktok: { bg: 'bg-slate-100 dark:bg-slate-700', text: 'text-slate-700 dark:text-slate-300', dot: 'bg-slate-900 dark:bg-white' },
    linkedin: { bg: 'bg-blue-50 dark:bg-blue-900/30', text: 'text-blue-600 dark:text-blue-400', dot: 'bg-blue-500' }
  }
...
const getStatusBadge = (status: ContentItem['status']) => {
  const configs: Record<typeof status, { bg: string; text: string }> = {
    viral: { bg: 'bg-green-50 dark:bg-green-900/30', text: 'text-green-600 dark:text-green-400' },
    active: { bg: 'bg-primary-50 dark:bg-primary-900/30', text: 'text-primary-600 dark:text-primary-400' },
    ended: { bg: 'bg-slate-100 dark:bg-slate-700', text: 'text-slate-500 dark:text-slate-400' }
  }
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
          ...
          >
            <td class="px-6 py-4 flex items-center gap-3">
              ...
              />
              <span class="font-bold text-slate-900 dark:text-white group-hover:text-primary-600 transition-colors">
                {{ item.title }}
              </span>
            </td>
            <td class="px-6 py-4">
              ...
              >
                <span :class="['block size-1.5 rounded-full', getPlatformBadge(item.platform).dot]" />
    ...
    <div class="md:hidden flex flex-col divide-y divide-slate-100 dark:divide-slate-700">
      ...
      >
        ...
        <div class="flex flex-col justify-center items-end text-right">
          <span class="text-lg font-bold text-slate-900 dark:text-white">{{ item.reach }}</span>
          <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">Reach</span>
        </div>
      </div>
    </div>
  </div>
