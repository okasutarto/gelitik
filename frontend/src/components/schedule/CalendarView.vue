...
const getPostsForDay = (date: Date) => {
  return currentMonthPosts.value.filter((post) => post.date.toDateString() === date.toDateString());
};
...
const onDrop = (e: DragEvent, _date?: Date) => {
  e.preventDefault();
...
<template>
  <div class="neo-card border-neo-3 border-black overflow-hidden">
    <!-- Weekday Headers -->
    <div class="grid grid-cols-7 bg-slate-50 dark:bg-slate-800/50 border-b-4 border-black">
      <div
    ...
    <!-- Calendar Grid -->
    <div class="grid grid-cols-7 auto-rows-fr bg-slate-200 dark:bg-slate-700 gap-1">
      <div
        ...
        :class="[
          ...
          isToday(day.date) && 'ring-1 ring-inset ring-primary-500 bg-primary-50/10',
        ]"
        @dragover="onDragOver"
      ...
      >
        ...
        <!-- Posts List -->
        <div class="space-y-1">
          <PostItem v-for="post in getPostsForDay(day.date)" :key="post.id" :post="post" />
        </div>
      </div>
    </div>
  </div>
